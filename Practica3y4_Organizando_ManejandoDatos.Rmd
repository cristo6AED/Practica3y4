...
pew_tidy <- gather(
  ...
  value = "freq",
  -religion,
  factor_key = TRUE  # para que 'income' quede como factor
)
...
names_col <- grep("wk", colnames(raw), value = TRUE)

my_billboard <- raw %>% 
  pivot_longer(col=names_col, names_to="week", values_to="rank",
               names_prefix = "wk",values_drop_na = TRUE) %>%
  mutate(
    week = as.integer(week),    # convierte la semana a número
    date = as.Date(date.entered) + weeks(week - 1)  # calcula fecha de esa semana
  )
...
```{r}
#colnames(raw)
names_col <- grep(pattern = "^(m|f)", x = colnames(raw), value = TRUE)
names_col
my_tuberculosis <- raw %>%
  gather(key = "demografia", value = "casos", names_col, na.rm = TRUE) %>%
  separate(col = demografia, into = c("sexo", "edad"), sep = 1)

head(my_tuberculosis)
```
...
weather_tidy <- raw %>%
  pivot_longer(cols = starts_with("d"), 
               ...
               values_drop_na = TRUE) %>%
  pivot_wider(names_from = element, values_from = temperatura) %>%
  mutate(fecha = as.Date(paste(year, month, gsub("d", "", dia), sep = "-"))) %>%
...
cat("flights contiene detalles de vuelos y retrasos, mientras que planes tiene detalles sobre los aviones, como su modelo, fabricante y capacidad. Ambos data frames se pueden combinar usando una clave común, que sería plane_id, ya que cada vuelo de flights probablemente está asociado con un avión de planes.")
...
planes.f <- planes %>%
  # 1️⃣ Eliminar los aviones de McDonnell Douglas (cualquier denominación posible)
  filter(!grepl("MCDONNELL|McDonnell", manufacturer, ignore.case = TRUE)) %>%
  
  # 2️⃣ Seleccionar los aviones con número de asientos entre 90 y 210
  filter(seats >= 90 & seats <= 210) %>%
  
  # 3️⃣ Unificar denominaciones del fabricante AIRBUS
  mutate(manufacturer = ifelse(grepl("AIRBUS", manufacturer, ignore.case = TRUE),
                               "AIRBUS COMP",
                               manufacturer))


planes.f %>% count(manufacturer)
...
```{r }
flights <- flights %>%
            select(-time_hour, -starts_with('time'))
```
