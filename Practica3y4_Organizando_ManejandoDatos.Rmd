...
#Hay un mcdonnel que está mal escrito en inglés,
#usar filter str_detect

planes.f <- planes %>%
              filter(str_detect(str_to_lower(manufacturer), '*.douglas.*'), negate=TRUE) %>%
              filter(between(seats, 90, 210)) %>%
              mutate(manufacturer = ifelse(str_detect(str_to_lower(manufacturer), '*.douglas.*')), 'AIRBUS COMP', manufacturer))

#-> %y POSIX -> strftime(as.Date('01-01-69'), format="%d-%m-%y", "%Y")
#antes del 69 es del 1900, depsués es 2000
```
